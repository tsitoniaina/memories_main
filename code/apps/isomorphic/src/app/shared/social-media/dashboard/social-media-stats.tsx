...
export default function SocialMediaStats({
  ...
  return (
    <div className={className}>
      <div className="grid grid-cols-1 gap-4 @2xl:grid-cols-3 @3xl:gap-6">
...
function StatCard({ className, statItem }: StatCardProps) {
  ...
  return (
    <div
      className={cn(
        'group relative inline-block w-full overflow-hidden rounded-lg border border-muted p-4 first:border-0 first:bg-[#E7F3E7] @3xl:p-6',
        className
      )}
    >
      <span className="absolute end-0 top-0 hidden size-20 -translate-y-10 translate-x-4 rotate-45 rounded-3xl bg-[#34C759]/15 group-first:block" />

      <div className="flex items-center gap-2">
        <Icon className="size-6" />
        <Text className="font-semibold text-gray-900 dark:group-first:text-gray-0">
          {title}
        </Text>
      </div>

      <div className="mb-2 flex items-center justify-between border-b border-muted py-3">
        <Text className="text-gray-900 dark:group-first:text-gray-0">
      ...
      <div className="flex items-center gap-2">
        ...
        <Text className="font-bold text-gray-900 dark:group-first:text-gray-0">
          +{formatNumber(likes)}
        </Text>
        likes/week
      </div>
